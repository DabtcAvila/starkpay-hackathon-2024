# ORG-006: Subir foto del equipo o avatar oficial

**Puntos:** 5  
**Estado:** 📸 PENDIENTE  

## Evidencia

El equipo StarkPay - ITAM necesita una foto oficial del equipo o avatar representativo.

### Opciones de Foto del Equipo

#### Opción 1: Foto Grupal Presencial
- **Ubicación:** [Lugar del meetup/oficina]
- **Fecha sugerida:** [Fecha]
- **Concepto:** Foto profesional con branding StarkPay
- **Props:** Laptops, logos, material del hackathon

#### Opción 2: Avatar Virtual del Equipo
- **Concepto:** Ilustración digital del equipo
- **Estilo:** Minimalista, consistente con branding
- **Elementos:** Lightning bolt, colores corporativos
- **Herramienta:** Figma/Canva/AI

#### Opción 3: Collage Virtual
- **Individual:** Fotos de cada miembro
- **Composición:** Grid 2x2 con logo central
- **Background:** Gradiente StarkPay
- **Texto:** "StarkPay - ITAM - Starknet Hackathon 2024"

### Especificaciones Técnicas
- **Resolución:** Mínimo 1920x1080
- **Formato:** JPG/PNG
- **Aspecto:** 16:9 o cuadrado (1:1)
- **Tamaño:** < 5MB para GitHub

### Ubicación Final
La foto será subida a:
```
evidence/organizational/team_photo.jpg
```

### Estado Actual
- ❌ Foto no disponible aún
- 📅 Coordinar sesión fotográfica
- 🎨 Crear versión digital alternativa

### Propuesta de Foto Digital

Si no es posible reunión presencial, crear avatar digital:

```
🧑‍💻 David (Tech) | 💼 Business Lead
                  StarkPay - ITAM
⚡ Lightning payments made simple
      🎨 Design Lead | 🎤 Pitch Lead
```

### Próximos Pasos
1. **Coordinar reunión** para foto grupal
2. **Crear avatar digital** como backup
3. **Subir imagen final** al repositorio
4. **Actualizar README** con foto del equipo

**NOTA:** Esta evidencia requiere acción inmediata para completar los puntos organizacionales.